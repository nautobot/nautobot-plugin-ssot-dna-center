...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SSOT_DNA_CENTER_xxx
namespace = Collection("nautobot_ssot_dna_center")
namespace.configure(
    {
        "nautobot_ssot_dna_center": {
            "nautobot_ver": "2.0.0",
            "project_name": "nautobot-ssot-dna-center",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_ssot_dna_center.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_ssot_dna_center.compose_http_timeout,
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_ssot_dna_center.project_name}",
        f'--project-directory "{context.nautobot_ssot_dna_center.compose_dir}"',
    ]
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_ssot_dna_center.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_ssot_dna_center.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_ssot_dna_center"
...
@task(
    help={
        "version": "Version of Nautobot SSoT for DNA Center to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_ssot_dna_center'
    run_command(context, command)
...
def unittest(
    context,
    keepdb=False,
    label="nautobot_ssot_dna_center",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_ssot_dna_center/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_ssot_dna_center.local):
